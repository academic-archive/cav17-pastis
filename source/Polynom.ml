...
module MkFactor(Pol: Poly) = struct
  ...
  let subst id p = function
    | Var v when v = id -> p
    | Max p' -> Pol.subst id p p'
    | Var v' -> Pol.of_var v'

end

module MkMonom(Pol: Poly)(Fac: Factor with type poly = Pol.t) = struct
  ...
  *)
  ...
  let fold = M.fold
  let one = M.empty
  let lift factor = M.add factor 1 one
  let of_var v = lift (Fac.of_var v)
...
module MkPoly(Mon: Monom) = struct
  ...
  *)
  ...
  type t = float M.t and monom = Mon.t

  let zero = M.empty
  let const k = M.add Mon.one k zero
  let compare = (M.compare compare: t -> t -> int)
  let fold = M.fold
  let of_var v = M.add (Mon.of_var v) 1. zero
  ...
  let subst id p p' =
    ...
    fold begin fun m k res ->
      ...
      (* Because we will always be using this module in
         ...
         at runtime.
      *)
      add_scale k (Obj.magic (Mon.subst id p m)) res
